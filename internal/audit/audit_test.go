...
func TestFileObserver_OmitsEmptyUserID(t *testing.T) {
	...
	// user_id omitempty — поле не должно попасть в JSON
...
func TestFileObserver_BadPath_NosPanic(t *testing.T) {
...
func TestHTTPObserver_BadURL_NoPanic(t *testing.T) {
	obs := NewHTTPObserver("http://127.0.0.1:1") // нет сервера на этом порту
	// не должно паниковать
	obs.Notify(Event{Action: "follow", URL: "https://x.com"})
}
...
// mockObserver собирает полученные события
...
func TestAuditor_NoObservers_NoPanic(t *testing.T) {
	a := NewAuditor()
	// подписчиков нет — не должно паниковать
...
// --- Auditor + FileObserver интеграция ---
...
// --- Auditor + HTTPObserver интеграция ---
